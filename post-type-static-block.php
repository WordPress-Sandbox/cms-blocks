...
class StaticBlockContent {
      ...
      register_post_type( 'static_block',
        array(
          'labels' => array(
            ...
            'all_items'           => __( 'All Content Blocks' ),
  ...
  static function get_static_content( $args=array() ) {
    ...
    if ( is_null($page_data) )
      ...
      $text = '<div id="content-block-' . $id . '" class="content-block '. $class .'"'. $style. '>'. $content .'</div>';
      $content = $text;
      ...
      if ($showtitle) $content =  '<'. $args->titletag .' class="content-block-title">'. $page_data->post_title .'</'. $args->titletag .'>' . $content;
...
function add_box_static_blocks() {
  global $meta_box_static_blocks;

  add_meta_box($meta_box_static_blocks['id'], $meta_box_static_blocks['title'], 'show_box_static_blocks', $meta_box_static_blocks['page'], $meta_box_static_blocks['context'], $meta_box_static_blocks['priority']);

}

add_action( 'admin_menu', 'add_box_static_blocks' );


// Callback function to show fields in meta box
function show_box_static_blocks() {
  global $meta_box_static_blocks, $post;

  // Use nonce for verification
  echo '<input type="hidden" name="blocks_meta_box_nonce" value="', wp_create_nonce( basename(__FILE__) ), '" />';
...
add_action( 'save_post', 'save_data_static_blocks' );


// Save data when post is edited
function save_data_static_blocks($post_id) {
  global $meta_box_static_blocks;

  // verify nonce
  if ( !isset($_POST['blocks_meta_box_nonce']) || !wp_verify_nonce( $_POST['blocks_meta_box_nonce'], basename(__FILE__) ) ) {
    return $post_id;
  }
...
// Add Widget to WP
add_action( 'widgets_init', 'static_block_widget' );

// Register Widget with WP
function static_block_widget() {
  register_widget( 'StaticBlock_Widget' );
}

// Custom widget class extending WP_Widget
class StaticBlock_Widget extends WP_Widget {

  function __construct() {

    $widget_ops = array( 'classname' => 'static-block', 'description' => __( 'Displays the content from a Content Block.' ) );
    $control_ops = array( 'width' => 300, 'height' => 350, 'id_base' => 'static-block-widget' );
    parent::__construct( 'static-block-widget', __( 'Content Block' ), $widget_ops, $control_ops );
  }

  function widget( $args, $instance ) {
    extract( $args );

    $id = (isset($instance['static_block_id'])) ? $instance['static_block_id'] : '';
  ...
  function update( $new_instance, $old_instance ) {
    ...
    $instance['static_block_id'] = (isset($new_instance['static_block_id'])) ? $new_instance['static_block_id'] : '';
